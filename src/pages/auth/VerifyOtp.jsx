...
const VerifyOtp = () => {
  ...
  const handleOtpResend = async (e) => {
    ...
    setResend(true);
    if(phone_number){
        const res = await axios.post('https://chronic-care-gpt.onrender.com/api/v1/auth/resendOtp/', { 'user': parseInt(userId, 10), 'phone_number': phone_number });
        const resp = res.data;
    }
}
  const handleOtpSubmit = async (e) => {
    ...
    if (otp.length === 6 && /^\d+$/.test(otp)) {
      ...
      try {
        const res = await axios.post('https://chronic-care-gpt.onrender.com/api/v1/auth/verify-phone/', { 'user': userInt, 'otp': otp });
        const resp = res.data;
        if (res.status === 200) {
